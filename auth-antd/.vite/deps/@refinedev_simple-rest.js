...
var q = (r, s = h) => ({ getList: async ({ resource: e, pagination: t, filters: o, sorters: n, meta: d }) => {
  let i = `${r}/${e}`, { current: c = 1, pageSize: a = 10, mode: p = "server" } = t ?? {}, { headers: $, method: m } = d ?? {}, x = m ?? "get", v = g(o), u = {};
  ...
  let _ = { ...u, ...v }, O = Object.keys(_).length ? `${i}?${(0, import_query_string2.stringify)(_)}` : i, { data: w, headers: F } = await s[x](O, { headers: $ }), S = +F["x-total-count"];
  ...
  if (n) {
    let m = f(n);
    if (m) {
      let { _sort: x, _order: v } = m, u = { _sort: x.join(","), _order: v.join(",") };
